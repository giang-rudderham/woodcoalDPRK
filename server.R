...
shinyServer(function(input, output) {
    ...
    leaflet () %>%
      ...
      addMarkers(lng = 125.8, lat = 38.97, 
                 popup = paste("The Capital: Pyongyang <br> Population Density:",
                               ...
                               "Households that use wood and coal as cooking fuel:",
                               dprk.shp$percWoodCoalCooking[dprk.shp$Display == "Pyongyang"], "%",
                               "<br>", "Households that use wood and coal for heating:",
                               dprk.shp$percWoodCoalHeating[dprk.shp$Display == "Pyongyang"], "%")
      )
  })  
  ...
  observe({
    ...
    proxy %>% clearGroup(group = "woodcoalcooking")
    if (input$cooking) {
      proxy %>%
        addCircles(data = dprk.shp, lng = ~long, lat = ~lat, weight = 1, opacity = 3,
                   radius = sqrt(dprk.shp$percWoodCoalCooking / pi) * 3000, 
                   popup = paste("Province:", dprk.shp$Display, "<br>",
                                 "Households that use wood and coal as cooking fuel:",
                                 dprk.shp$percWoodCoalCooking, "%"),
                   group = "woodcoalcooking", layerId = ~paste0(Display, "1")
  ...
  observe({
    proxy <- leafletProxy("dprkmap", data = dprk.shp)
    proxy %>% clearGroup(group = "woodcoalheating")
    if (input$heating) {
      proxy %>%
        addCircles(data = dprk.shp, lng = ~long, lat = ~lat, weight = 1, opacity = 3,
                   radius = sqrt(dprk.shp$percWoodCoalHeating / pi) * 3000,
                   fillColor = "green", color = "green",
                   popup = paste("Province:", dprk.shp$Display, "<br>",
                                 "Households that use wood and coal for heating:",
                                 dprk.shp$percWoodCoalHeating, "%"),
                   group = "woodcoalheating", layerId = ~paste0(Display, "2")
...
#   ## clicking on a province generates bar plots of heating and cooking fuel for that province
# 
#   #observeEvent(input$dprkmap_shape_click, { #doesn't work
#    # p <- input$dprk_shape_click
#     #if (is.null(p$id)) {"Click on a province to display bar plots"}
#   #})
#   
#   df_reactive <- eventReactive(input$dprkmap_shape_click, {
#     p <- input$dprkmap_shape_click
...
# Version 2: update map with clicking on the provinces 
#######################################################
